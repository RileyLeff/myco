...
- **`deriv` output must not feed back into the SCC it differentiates through.**
  ...
  separate components.
  **Soul alignment note:** this restriction is an intentional abstraction leak.
  It requires the model author to know whether a `deriv` expression participates
  in the same SCC as the quantities it differentiates — a solver-level concern.
  The tradeoff is accepted because the alternative (silent Hessian computation
  inside Newton loops) would produce silently wrong or divergent results.
...
(Eqn 8) requires the marginal carbon cost of water: chi_w = dG/dE. This
cannot be expressed as a native `.myco` `deriv` expression that feeds back
into `g_s`, because doing so would create a `deriv`-containing SCC cycle
(section 9.5). In the mock, the slot mechanism handles this: the slot is
trained to maximize growth, and the optimality condition emerges from training.
For analytical optimality models, the optimality rule belongs in the Python
workflow (computing the derivative externally and supplying it as data) or in
a neural slot that learns the optimal response surface.
