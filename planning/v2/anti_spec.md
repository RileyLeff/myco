...
| `property` declarations (`property sigma is PositiveDefinite`) | refinement types + capability contracts (`Invertible<_>`, `Differentiable`, `Monotone`) + `constraint` blocks | redundant with existing machinery; canonical spec §6 already forbids user property-declaration surface. mock_sperry.myco flagged for rewrite |
...
| caller-scope capture in parameterized-relation bodies | explicit formal parameters, local `let`s, imports, universals, declared relations, and stdlib atoms only | reusable graph structure must expose every constrained value at the call site |
| direct or indirect recursive parameterized-relation invocation in v2.1 | ordinary graph cycles, temporal dynamics, or events | syntactic unfolding recursion would duplicate graph structure without a termination discipline |
...
| `~<Ito>` / `~<Stratonovich>` stochastic-operator sugar | convention type parameter on the stochastic family, e.g. `BrownianMotion<Ito>` | `~` remains the unparameterized distribution-binding operator; convention belongs to the stochastic process family |
| separate stochastic operators for aleatoric vs epistemic uncertainty (`~` / `~~`, or similar) | one `~` distributional attachment plus graph / workflow-derived classification | epistemic vs aleatoric role is experiment-relative and can be set-valued in hierarchical models |
| source-level stochastic-kind annotations (`epistemic(x, D)`, `aleatoric(x, D)`, attributes, etc.) | compiler classification surfaced by `hypha explain` | user-marked kind surfaces drift from workflow truth and force experiment facts into source |
| implicit record-level joint factoring or implicit joining of field-level `~` statements | explicit structured joint root or record-`~` sugar with named fields | stochastic coupling must name the e-class/root it lives on |
...
| distribution-valued optimality criteria treated as orderable | explicit projection through stdlib atoms such as `expected`, `quantile`, `cvar`, `pr_above`, or `entropy` | stochastic scalarization is a model commitment, not an implicit ordering |
| workflow-side selection of stochastic-criterion projection | projection lives in `.myco` as part of the criterion expression | mean, quantile, CVaR, probability, and entropy define different claims |
| `alternatives = interval(...)` as the only legal optimality choice-space form | typed `ChoiceSpace<T>` with `Interval`, `Finite`, `ProductSpace`, `MeasurableMap`, and catalog-scoped `ContractImplSet` | intervals are one primitive, not the whole alternatives ontology |
| `ContractImplSet<C>` treated as package-independent mathematical universe | catalog-scoped implementation set recorded with package graph, contract version, and implementations | loaded spores affect the set; evidence must make that scope explicit |
| automatic optimality penalty insertion when a `Trainable` binding overlaps an `OptimalitySite` | explicit `strict`, `train_then_verify`, `relaxed_training`, or `diagnostic_only` workflow policy recorded in the run ledger | preserves the asymmetry between value-producing bindings and evidence fulfillment; no compiler-inserted training terms |
| Python-callable closure policies as Y5 fulfillments | `.myco` parameterized relation satisfying the Y5 contract, selected by name in `run.config.closure_policy` | extraction-time commitments must be inspectable, contract-checked, and differentiable when required; opaque combiners route through ordinary `Controller` plus a world relation |
...
| implicit controller invocation scope inferred from path shapes | required axis-aware `scope` on `Controller`: `PerInstance`, `OverAxes`, or `Global` | per-instance, batched-over-axis, and global call semantics are load-bearing and must be explicit |
| controller bindings that read across collection axes without explicit `.myco` aggregation | write an ordinary `.myco` aggregation / mapping relation, then read the aggregate path | aggregation is a model claim, not a hidden workflow binding behavior |
| mutable catalog handles or hidden writes inside controller callables | immutable value payloads at declared `reads`; outputs accepted only at declared `writes` | hidden effects would make Python a second model layer |
| controller binding implying `OptimalitySite` fulfillment | controllers produce values; optimality evidence is separate (§15.7, §25) | preserves the value-producing binding vs evidence-fulfillment split |
| `.myco` syntax for FiLM, embeddings, taxonomic effects, or learned-policy architecture patterns | ordinary contracts plus workflow-side `Controller` callables | architecture-specific patterns belong in Python workflow code, checked against the catalog |
| `observe` overloaded to mean probabilistic conditioning or deterministic residual target depending on whether `~` is present | `observe` for distributional conditioning; `fit_to` for deterministic residual targets | workflow intent must be readable at the call site |
| probabilistic `observe` without compatible distributional structure on the target | source `~`, compatible `Prior`, `ProcessPrior`, or another explicit distributional workflow source | likelihood must not appear from nowhere |
| same target e-class receiving both probabilistic `observe` and deterministic `fit_to` in one workflow | choose one evidence path; residual-style fitting of stochastic models is an inference recipe over the distributional site | avoids incompatible structural commitments on one path |
| silent conflict resolution between source `~` and workflow `Prior(D)` | compatible refinement or workflow-composition error | workflow distributional bindings may not override source claims silently |
| deterministic `fit_to` without explicit data/path shape alignment | catalog-validated alignment or workflow-composition error | prevents silent broadcasting or replication of target data |
...
| default Itô interpretation for state-dependent SDEs | explicit stochastic-family convention such as `BrownianMotion<Ito>` or `BrownianMotion<Stratonovich>` | SDE convention is a world claim when it changes the math; omission is allowed only when the compiler proves the site convention-irrelevant |
...
| implicit least-squares / hidden objective for mismatched data | unhandled-residual workflow-composition error unless the workflow selects `observe`, `fit_to`, exact conditioning, penalty, projection, inference, or explicit ignore policy | mismatched data is normal; pretending a default objective is scientific intent is not |
...
| uncertified early stopping inside exact Y6 | exact Y6 uses only proof-preserving reductions; guided search is a separate workflow-authorized approximation | users must know whether closure is exhaustive or approximate |
| magic language-wide Y6 blowup threshold | active workflow budgets plus diagnostics reporting raw / reduced subsystem counts | computational appetite belongs in workflow configuration, not source semantics |
