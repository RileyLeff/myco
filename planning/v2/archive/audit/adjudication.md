...
| H6 | "Kernels-as-functions" architectural decision not stated | anti_spec entry "no `kernel` keyword" or sentence in §6 | ACC/W | Superseded by chunk 08 wording: kernels are parameterized relations with explicit output slots, not a separate `kernel` keyword or ordinary user `fn` surface. anti_spec.md records the retired keyword/kind. |
...
| ID | Finding | Recommendation | Status | Notes |
|---|---|---|---|---|
| H1 | Initialization mechanisms and mutual exclusion missing | add §9.x; largest gap in §9 | ACC/W | §9.3 Initialization added. Mechanisms are now expressed through `initial:` plus workflow `bind(path.initial, source)` source objects (`Constant`, `Series`, `Trainable`, `Prior`) rather than the retired `assume_initial` / `learn_initial` / `learn_trajectory` verbs. Mutual-exclusion and missing-initial diagnostics are canonical. |
...
| H3 | Workflow-side epistemic prior API unconnected from §13.1 | add sentence cross-referencing §24 source model | ACC/W | §13.1 now cross-links epistemic uncertainty to workflow-side `Prior(distribution)` source objects attached with `bind(path, source)` (§24.2), which attach distributional facts to the e-class at inference/training time. |
...
| C1 | `loss_of` field inventory conflict: §14.2 uses `{data_fit, constraint_violation, regularization}`; O2.4 locks `{compute, approximation, condition, truncation, discretization}` | resolve overload: one struct, two structs, or rename one intrinsic. Document and update anti_spec | ACC/W | Resolved by chunk 12: `cost_of(expr)` owns extraction economics; `objective_terms(residual)` owns training-objective decomposition; `loss_of` retired in anti_spec.md. Applied 2026-04-23 |
...
| H4 | Multi-dimensional extraction-cost record absent; training-objective fields not cross-linked | cross-ref §15.2 to §14.2 named-field objective/cost structs | ACC/W | §14.2 / §19.1 define `cost_of`; §14.2 / §25 define `objective_terms`. |
...
| H6 | Workflow-side sampling parameters (`run.config.approximation_estimation`) absent | add sentence to §15.2 noting sampling config is workflow-side per CC1; cross-ref §24 | ACC/W | §15.2 states empirical error-estimation parameters live workflow-side; §24.5 names `run.config.approximation_estimation`. |
...
| H11 | Workflow config for inspection / approximation-estimation policy not cross-linked | cross-ref §24 | ACC/W | §24.5 names `run.config.approximation_estimation`; inspection remains workflow/tooling policy. |
...
| ID | Finding | Recommendation | Status | Notes |
|---|---|---|---|---|
| H1 | `run.config.extraction_policy` and objective/approximation estimation fields absent from §24.5 representative fields | add fields or cross-ref §19 / §14 | ACC/W | §24.5 now includes `run.config.extraction_policy`, `run.config.objective_policy`, and `run.config.approximation_estimation`. |
...
- **Cost-field struct unification** (§14 C1 + §15 H4/H5 + §19 H2/C2). RESOLVED 2026-04-23. `cost_of(expr)` owns extraction economics; `objective_terms(residual)` owns training-objective decomposition; `loss_of` is retired.
...
| H7 | Training-mode consistency-objective substitution rule (`lhs = rhs` → `(lhs - rhs)²`) | one-line statement in §25 | ACC/W | §25 now names Appendix C O1. |
...
| H9 | §25 aggregation presumes training-objective named fields (cost-field unification dependency) | resolve with chunk 12 vocabulary | ACC/W | §25 now consumes `objective_terms(residual)` rather than `loss_of`. |
...
| C1 | §26.2 authorizes `Float64 -> Float32` via bare `convert`; chunks 04/05 require `approximate ... tolerance_class` | decide single authorizing surface | ACC/W | Precision downcasts (`Float64 -> Float32` and tensor element-type analogues) require an authorizing `approximate` block with precision-downcast tolerance class; bare `convert` is not enough. Applied 2026-04-23 |
...
| C3 | §27.3 "catalog is closed" contradicted by chunk 04 promoted NormalInverseGamma + Gamma-Gamma | covered by cross-cutting below | ACC/W | Gamma-Gamma ships in §27.3. Normal-InverseGamma remains gated on joint-prior rewrite-pattern language and is tracked as Tier 2 / joint-family work. Applied 2026-04-23 |
...
| ID | Finding | Recommendation | Status | Notes |
|---|---|---|---|---|
| H1 | Compact-support / characteristic-length declaration surface (contract vs refinement vs workflow) | three-option bullet in §28.3 or §35 | ACC/W | Chose capability-contract surface: `CompactSupport(radius)` on the kernel relation/family. Not a workflow annotation and not an output refinement. Applied 2026-04-23 |
...
- **Precision-downcast authorizing surface** (§26 C1). RESOLVED. Bare `convert` does not authorize precision downcast. Scalar and tensor precision downcasts require an `approximate` block with precision-downcast tolerance class.
...
| C1 | §31 summary pre-commits autodiff as minimum-API responsibility while §32 lists AD ownership as open (implicitly assumes Options B or C) | qualify summary: autodiff joins minimum API under hybrid/full-delegation | ACC/W | Hybrid AD boundary locked: Myco owns symbolic/algorithmic derivatives; backends own runtime AD over emitted kernels and opaque callables. Applied 2026-04-23 |
...
| C3 | §31.1 "Conservative default" for `error` fallback vs chunk 06 §4.2 "Open question: default policy" | formally lock `error` as default OR move to §32 as explicit open | ACC/W | `error` is locked as the default fallback mode; `host` and `emulate` are explicit run-config opt-ins. Applied 2026-04-23 |
...
| H3 | `Controller` opaque-callable fallback (non-differentiable callable in training-time SCC) residue | add §32.5 bullet: stop-gradient / compile error / workflow opt-in | O/W | lean same-backend already in §32.1; fallback is the residue |
...
| C2 | anti_spec.md `Dual` retirement reason "Part V commits backend-delegated AD" implies Option B while §32 leans Option C | rewrite retirement reason option-neutral: "redundant with backend AD in all three options; risks conflicting representation" | ACC/W | anti_spec.md now points to the hybrid AD boundary instead of pure backend delegation. Applied 2026-04-23 |
...
| H6 | Chunk 12 (cost-field unification) dependency on locked chunk 04 O2.4 `cost_of` five-field struct not stated | add sentence clarifying resolved field split | ACC/W | Chunk 12 report now records the resolved six-field `cost_of` record and separate `objective_terms`. |
| S1 | chunk 03 §2 "kernels are ordinary `.myco` functions" framing | superseded by chunk 08 "parameterized relations" lock; anti_spec.md row still uses interim "ordinary `fn` accepting two point arguments" framing | ACC/W | anti_spec.md replacement text and §28 now use "parameterized relation" for kernels. Applied 2026-04-23 |
...
| C1 | §34 Summary enumerates chunk 12 as open after resolution | keep chunk 12 only as completed reference | ACC/W | §34 now marks chunk 12 resolved, not outstanding. |
...
- **AD-ownership fork disposition** (§31 H1 + §31 C1 + §32 H1 + §32 C2 + §33 H4). RESOLVED. §31 locks the hybrid boundary: Myco owns symbolic/algorithmic derivatives; backends own runtime AD over emitted kernels and opaque callables. anti_spec.md's `Dual` retirement now points to that hybrid boundary, not pure backend delegation.
...
- **Fallback-default-mode formal lock** (§31 C3). RESOLVED. `error` is the default fallback mode; `host` and `emulate` are explicit workflow opt-ins.
...
- **"Kernels as ordinary functions" anti_spec.md row staleness** (§34 S1). RESOLVED. Kernels are parameterized relations with explicit output slots; the ordinary-`fn` replacement wording is retired.
...
| H3 | Workflow source-object taxonomy grouping question | add short open item; revisit after §24 source model is stable | SKIP | resolved by the three-verb source model: `bind`, `bind_topology`, `observe` plus source objects |
...
- **CLI binary naming resolved: `hypha`** (§36 H1 + §36 C1 + §36 H4 + §37 H6 + §39 H1). RESOLVED. `hypha` is the single user-facing CLI covering compile, run, check, fmt, explain, doc generation, and package-management subcommands. Whether an internal compiler binary exists is implementation detail, not spec prose.
...
| H10 | `objective_terms` (§14.2) and `cost_of` compiler intrinsics alongside `deriv`/`integrate`/`condition_of` but missing from stdlib-reserved | add `objective_terms` and `cost_of` to stdlib-reserved list | ACC/W | Appendix A reserves `objective_terms` and `cost_of`; `loss_of` is retired. |
...
| C2 | `identify` listed under Body-form keywords alongside `let`/`if`; actually appears as geometry-body / module-scope declaration per §11.2 and §17 X2 — not a general body form | move `identify` to Declaration keywords, OR add parenthetical clarifying scope ("geometry-body / module-scope only") | ACC/W | `identify` is listed under declaration keywords, not body-form keywords. Applied 2026-04-23 |
